...
namespace HowMuchTo.Models
{
    class ChallengeRepoDapper : IChallengeRepository
    {
        ...
        public void SetTargetPrice(long ChallengeID, decimal Price)
        {
            using (SqlConnection db = new SqlConnection(connStr))
            {
                db.Open();
                int numAffected = db.Execute("spChallengeSetTargetPrice", new { ChallengeID = ChallengeID, TargetPrice = Price }, commandType: CommandType.StoredProcedure);
            }
        }

        public Challenge CheckForDuplicate(Challenge item)
        {
            using (SqlConnection db = new SqlConnection(connStr))
            {
                db.Open();
                var chal = db.Query<Challenge>("spChallengeFindExisting", new { Description=item.Description, CustomerID=item.CustomerID, Privacy=item.Privacy, Date=item.Created }, commandType: CommandType.StoredProcedure);
                return chal.FirstOrDefault<Challenge>();
            }
        }
    }
}
