...
namespace HowMuchTo.Models
{
    public class AccountRepository : IAccountRepository
    {
        ...
        public bool ModifyCustomerAccountBalance(long CustomerID, decimal Amount)
        {
            using (SqlConnection conn = new SqlConnection(connStr))
            {
                conn.Open();
                int numRows=conn.Execute("ModifyCustomerAccountBalance", new { CustomerID = CustomerID, Amount = Amount }, commandType: CommandType.StoredProcedure);
                return (numRows > 0);
            }
        }
