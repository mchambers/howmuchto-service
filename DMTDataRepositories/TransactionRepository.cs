...
namespace HowMuchTo.Models
{
    public class TransactionRepository : ITransactionRepository
    {
        ...
        public string RecordTransactionBatch(List<Transaction> Transactions, string TransactionGroupID = "")
        {
            ...
            using (SqlConnection conn = new SqlConnection(connStr))
            {
                ...
                foreach (Transaction t in Transactions)
                {
                    ...
                    conn.Execute("spTransactionRecordAndProcess", TransToDynParm(t), dbTrans, commandType: CommandType.StoredProcedure);
                }
        ...
        public IEnumerable<Transaction> TransactionsForCustomer(long CustomerID, int StartAt = 0, int Amount = 10)
        {
            using (SqlConnection db = new SqlConnection(connStr))
            {
                db.Open();
                var trans = db.Query<Transaction>("spTransactionGetForCustomer", new { CustomerID=CustomerID, StartAt = StartAt, Amount = Amount }, commandType: CommandType.StoredProcedure).AsEnumerable();
                return trans;
            }
        }

        public IEnumerable<Transaction> TransactionsForBid(long BidID)
        {
